...
void ticks() {
  ...
  static const uint16_t stocksSyncInterval=900;
  ...
  static uint16_t stocksSyncTicks = 0;
  static bool divrssi;
  timeSyncTicks++;
  weatherSyncTicks++;
  divrssi = !divrssi;
  if(network.status == CONNECTED){
    if(network.forceTimeSync || network.forceWeather || network.forceStocks){
    ...
    if(stocksSyncTicks >= stocksSyncInterval){
      stocksSyncTicks=0;
      network.forceStocks = true;
    }
  }
#ifndef DSP_LCD
...
void MyNetwork::setWifiParams(){
  ...
  #endif
  #if (DSP_MODEL!=DSP_DUMMY || defined(USE_NEXTION))
    stocksBuf = (char *) malloc(sizeof(char) * STOCKS_STRING_L);
    memset(stocksBuf, 0, STOCKS_STRING_L);
  #endif
...
void MyNetwork::requestStocksSync(){
  display.putRequest(NEWSTOCKS);
}
...
void doSync( void * pvParameters ) {
  ...
  if(network.stocksBuf && config.store.showstocks && network.forceStocks){
    network.forceStocks = false;
    bool ok = getStocks(network.stocksBuf);
    if(ok) network.requestStocksSync();
  }
  vTaskDelete( NULL );
}
...
bool getStocks(char *sbuf) { (void)sbuf; return false; }
